...
export function createAdult() {
  return (dispatch, getState) => {
    ...
    // 检查每一个passenger的属性是否有空的，如果有则直接返回
...
export function removePassenger(id) {
  return (dispatch, getState) => {
    ...
    const newPassengers = passengers.filter((passenger) => {
      // 删去passenger.followAdult !== id的passenger的意思是 如果某成人passenger被去除，与之随行的儿童也必须被去除
...
export function onUpdatePassenger(id, data, keysToBeRemoved = []) {
  return (dispatch, getState) => {
    const { passengers } = getState();
...
export function showGenderMenu(id) {
  return (dispatch, getState) => {
    ...
    dispatch(
      showMemu({
        onPress(gender) {
          dispatch(onUpdatePassenger(id, { gender }));
          dispatch(hideMenu());
        },
        options: [
          {
...
export function showFollowAdultMenu(id) {
  return (dispatch, getState) => {
    ...
    dispatch(
      showMemu({
        onPress(followAdult) {
          dispatch(onUpdatePassenger(id, { followAdult }));
          dispatch(hideMenu());
        },
        options: passengers
